...
const initialNodes: Node[] = [
  ...
  {
    id: '2',
    data: { label: '⚛️ React 18 SPA' },
    position: { x: 100, y: 120 },
    style: nodeStyle,
  },
  {
    id: '3',
    data: { label: '🔄 Redux State' },
...
export default function FullStackFlow({ embedded = false }: { embedded?: boolean }) {
        ...
        <div className="absolute bottom-2 sm:bottom-4 left-2 sm:left-4 bg-black/60 backdrop-blur-md border border-white/10 rounded-lg sm:rounded-xl p-2 sm:p-4 text-[10px] sm:text-xs text-white/80 space-y-1 sm:space-y-2 max-w-[280px] sm:max-w-xs z-10">
          ...
          <div className="space-y-1 sm:space-y-1.5 text-white/70">
            <div className="flex items-start gap-2">
              ...
              <span>Browser makes HTTP request to API</span>
            </div>
            <div className="flex items-start gap-2">
              <span className="text-primary mt-0.5">2.</span>
              <span>React SPA handles routing & Redux state</span>
            </div>
            <div className="flex items-start gap-2">
              <span className="text-primary mt-0.5">3.</span>
              <span>Django REST API processes request</span>
            </div>
            <div className="flex items-start gap-2">
              <span className="text-primary mt-0.5">4.</span>
              <span>PostgreSQL or Redis returns data</span>
            </div>
            <div className="flex items-start gap-2">
              <span className="text-primary mt-0.5">5.</span>
              <span>JSON response updates React UI</span>
            </div>
          </div>
        </div>
      )}
