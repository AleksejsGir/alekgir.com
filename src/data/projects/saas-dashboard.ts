...
export const saasDashboard: Project = {
  ...
  fullDescription: "A production-ready SaaS dashboard single-page application built with React and Django REST Framework. Designed for multi-tenant architecture with tenant-specific data isolation and comprehensive access control.\n\nKey features include interactive data visualizations with Chart.js and Recharts, real-time updates via Django Channels WebSocket, customizable widgets, role-based access control, and export functionality for reports (PDF, CSV).\n\nTechnical highlights: Django REST Framework API with ViewSets and JWT authentication, React 18 SPA with TypeScript and Redux Toolkit, Redis caching reducing API response time by 60%, and 80%+ test coverage across backend and frontend.",
