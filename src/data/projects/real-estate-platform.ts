...
export const realEstatePlatform: Project = {
  ...
  fullDescription: "A comprehensive real estate single-page application built with React and Django REST Framework. Designed for property discovery with advanced geospatial search capabilities and rich media experiences.\n\nKey features include radius-based property search with multiple filters, Google Maps integration with custom markers and clustering, 360° virtual tours, agent profiles with booking system for viewings, and property comparison tool.\n\nTechnical highlights: PostgreSQL PostGIS extension for geospatial queries achieving under 2s search time, Django REST Framework API with spatial serializers, React 18 with Google Maps JavaScript API, and Celery for automated booking notifications.",
