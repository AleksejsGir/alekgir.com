...
export const aiAnalyticsDashboard: Project = {
  ...
  fullDescription: "An advanced analytics single-page application leveraging React and async FastAPI. Designed for business intelligence with AI-powered insights and automated reporting capabilities.\n\nKey features include integration with OpenAI GPT-4 and Google Gemini for natural language analysis, interactive Plotly visualizations with drill-down functionality, automated PDF report generation with Celery scheduling, and CSV/Excel data export.\n\nTechnical highlights: async FastAPI with httpx for parallel AI API requests, pandas data processing pipeline with chunked operations for large datasets, PostgreSQL with asyncpg achieving sub-500ms query times, and Redis caching for AI response optimization.",
