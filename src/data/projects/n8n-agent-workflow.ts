...
export const n8nAgentWorkflow: Project = {
  ...
  fullDescription: "An advanced workflow automation platform built with n8n orchestrating complex AI agent interactions. Designed for enterprise-scale automation with visual workflow builder and custom node development.\n\nKey features include Model Context Protocol (MCP) integration for AI tool access, multi-agent coordination with OpenAI GPT-4 and Claude, real-time monitoring dashboard, and comprehensive error handling with automatic retries.\n\nTechnical highlights: custom Python nodes for specialized business logic, PostgreSQL for durable state persistence, Redis-based queue management with distributed locking, and containerized deployment achieving 99.5% uptime.",
